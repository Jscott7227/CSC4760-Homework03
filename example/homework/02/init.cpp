...
public:
  Domain(int _M, int _N, int _halo_depth, const char *_name = "") :
    ...
    interior("interior", M + 2 * halo_depth, N + 2 * halo_depth),
    exterior("exterior", M + 2 * halo_depth, N + 2 * halo_depth)
  {
    Kokkos::deep_copy(exterior, 0); // Initialize exterior to 0
    interior = Kokkos::subview(exterior, Kokkos::make_pair(halo_depth, M + halo_depth),
                                Kokkos::make_pair(halo_depth, N + halo_depth));
  }
  ...
  Kokkos::View<char **, Kokkos::LayoutRight> rawptr() { return exterior; }
  Kokkos::View<char **, Kokkos::LayoutRight> cookedptr() { return interior; }

protected:
  Kokkos::View<char **, Kokkos::LayoutRight> interior, exterior;
  int M, N, halo_depth;
  std::string name;
};
...
int main(int argc, char *argv[]) {
  if (argc < 6) {
    std::cout << "usage: " << argv[0] << " M N P Q iterations" << std::endl;
    exit(0);
  }
  ...
  if (0 == myrank) {
     ...
     P = std::atoi(argv[3]); Q = std::atoi(argv[4]);
  ...
  if (P * Q != size) {
    if (0 == myrank)
      std::cout << "grid mismatch: " << argv[0] << P << "*" << Q << " must be size of world: "<< size <<"!" << std::endl;
    
    exit(0);
  }
  
  // Assuming Process_2DGrid and parallel_code functions are defined elsewhere
...
Kokkos::parallel_for("update_cells", m, KOKKOS_LAMBDA(int i) {
